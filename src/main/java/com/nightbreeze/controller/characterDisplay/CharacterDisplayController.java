...
public class CharacterDisplayController implements Initializable {
    ...
    @FXML
    private Label characterNameLabel, raceClassLevelLabel, hpLabel;
    @FXML
    private ProgressBar hpProgressBar, xpProgressBar;
    @FXML
    private Label strScoreLabel, strModifierLabel, dexScoreLabel, dexModifierLabel, conScoreLabel, conModifierLabel;
    @FXML
    private Label intScoreLabel, intModifierLabel, wisScoreLabel, wisModifierLabel, chaScoreLabel, chaModifierLabel;
    @FXML
    private VBox savingThrowsBox, armorProficiencyBox, weaponProficiencyBox, toolProficiencyBox;
    @FXML
    private TextField healTextField, damageTextField, tempHpTextField, xpTextField;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (currentCharacter == null) {
            Utils.showErrorAlert("Load Error", "Could not load character data. Display is empty.");
            return;
        }
        refreshAllUi();
    }
    ...
    void handleSetTempHpButton(ActionEvent event) {
        try {
            ...
            currentCharacter.setTemporaryHP(amount);
    ...
    void handleAttackRollButton(ActionEvent event) {
        ...
        int modifier = getAbilityModifierFromString(selectedAbility);
        modifier += currentCharacter.getProficiencyBonus(); // Add proficiency to attacks
    ...
    private void updateXpUi() {
        ...
        xpLabel1.setText(String.format("XP: %d / %d", currentXp, nextLevelXp));
    ...
    private void populateHeader() {
        ...
        String subRace = currentCharacter.getSubRace() != null && !currentCharacter.getSubRace().isEmpty()
                ? currentCharacter.getSubRace() + " "
                : "";
        raceClassLevelLabel.setText(String.format(
                "(%s%s | %s %d)",
                subRace,
                currentCharacter.getRace(),
                currentCharacter.getClassName(),
    ...
    private void updateHpUi() {
        ...
        if (maxHp > 0) {
            hpProgressBar.setProgress((double) currentHp / maxHp);
        } else {
            hpProgressBar.setProgress(0.0);
        }
    }
